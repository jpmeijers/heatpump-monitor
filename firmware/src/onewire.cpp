...
bool readNextSensor()
{
  ...
  snprintf(myMessage.topic, sizeof(myMessage.topic), "/onewire/%s/temperature", sensorId);
  snprintf(myMessage.raw, sizeof(myMessage.raw), "%g", temperature);
...
void setupOneWire()
{
  // Start up the library - not necessary
  // sensors.begin();
}
